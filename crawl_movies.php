...
<div class="wrap">
    ...
    <a href="?page=crawl-apii-tools" class="nav-tab <?php if($tab===null):?>nav-tab-active<?php endif; ?>">Thủ công</a>
    <a href="?page=crawl-apii-tools&tab=schedule" class="nav-tab <?php if($tab==='schedule'):?>nav-tab-active<?php endif; ?>">Tự động</a>
    <a href="?page=crawl-apii-tools&tab=about" class="nav-tab <?php if($tab==='about'):?>nav-tab-active<?php endif; ?>">Giới thiệu</a>
  </nav>
	<div class="tab-content">
		<?php
			switch($tab) :
				case 'schedule':
				$crawl_apii_settings = json_decode(get_option(CRAWL_APII_OPTION_SETTINGS, []));
					...
					<div class="postbox">
						<div class="inside">
							...
							<div>
								...
								<p>
									Cấu hình crontab: <code><i style="color:blueviolet">*/10 * * * *</i> cd <i style="color:blueviolet">/path/to/</i>wp-content/plugins/crawl_apii_halimthemes/ && php -q schedule.php <i style="color:blueviolet">{secret_key}</i></code>
								</p>
								<p>
									Ví dụ:
									<br />
									Mỗi 5 phút: <code>*/5 * * * * cd <?php echo CRAWL_APII_PATH; ?> && php -q schedule.php <i style="color:blueviolet"><?php echo get_option(CRAWL_APII_OPTION_SECRET_KEY, ''); ?></i></code>
									<br />
									Mỗi 10 phút: <code>*/10 * * * * cd <?php echo CRAWL_APII_PATH; ?> && php -q schedule.php <i style="color:blueviolet"><?php echo get_option(CRAWL_APII_OPTION_SECRET_KEY, ''); ?></i></code>
								</p>
							</div>
						</div>
					</div>
				...
				<div class="crawl_page">
					<div class="postbox">
						<div class="inside">
							...
							<div>
								<p>
									Secret Key: <input type="text" name="crawl_apii_schedule_secret" value="<?php echo get_option(CRAWL_APII_OPTION_SECRET_KEY, ''); ?>">
									<button id="save_crawl_apii_schedule_secret" class="button">Lưu mật khẩu</button>
								</p>
							</div>
							<div>
								<p>
									Kích hoạt:
									<input type="checkbox" class="wppd-ui-toggle" id="crawl_apii_schedule_enable" name="crawl_apii_schedule_enable"
										value=""
										<?php echo (json_decode(file_get_contents(CRAWL_APII_PATH_SCHEDULE_JSON))->enable === true) ? 'checked' : ''; ?>
									>
								</p>
							</div>
							<div>
								<p>Trạng thái: <?php echo (int) get_option(CRAWL_APII_OPTION_RUNNING, 0) === 1 ? "<code style='color: blue'>Đang chạy...</code>" : "<code style='color: chocolate'>Dừng</code>"; ?></p>
							</div>
							<div>
								<p>Bỏ qua định dạng: <code style="color: red"><?php echo join(', ', $crawl_apii_settings->filterType);?></code></p>
								<p>Bỏ qua thể loại: <code style="color: red"><?php echo join(', ', $crawl_apii_settings->filterCategory);?></code></p>
								<p>Bỏ qua quốc gia: <code style="color: red"><?php echo join(', ', $crawl_apii_settings->filterCountry);?></code></p>
							</div>
							<div>
								<p>Page đầu: <code style="color: blue"><?php echo $crawl_apii_settings->pageFrom;?></code></p>
								<p>Page cuối: <code style="color: blue"><?php echo $crawl_apii_settings->pageTo;?></code></p>
							</div>
		...
		?>
				<div class="crawl_page">
					<div class="postbox">
						<div class="inside">
							Apii.Online là website dữ liệu phim miễn phí vĩnh viễn. Cập nhật nhanh, chất lượng cao, ổn định và lâu dài. Tốc độ phát cực nhanh với đường truyền băng thông cao, đảm bảo đáp ứng được lượng xem phim trực tuyến lớn. Đồng thời giúp nhà phát triển website phim giảm thiểu chi phí của các dịch vụ lưu trữ và stream. <br />
							...
							- API được cung cấp miễn phí: <a href="https://apii.online/docs" target="_blank">https://apii.online/docs</a> <br />
							- Tham gia trao đổi tại: <a href="https://t.me/+uGpf4TNyw1o5MTE1" target="_blank">https://t.me/+uGpf4TNyw1o5MTE1</a> <br />
						</div>
					</div>
				</div>
		<?php
		...
		?>
			<div class="crawl_main">
				<div class="crawl_filter notice notice-info">
					...
					<p>
						<div id="save_crawl_apii_schedule" class="button">Lưu cấu hình cho crawl tự động</div>
					</p>
				</div>
				...
				<div class="crawl_page">
					Hết thời gian chờ ngẫu nhiên: Từ  <input type="number" name="timeout_from" value="">(ms) - 
				...
				<div class="crawl_page">
					...
					<div id="crawl_movies" class="primary">Crawl phim</div>
...
add_action('wp_ajax_save_crawl_apii_schedule_secret', 'save_crawl_apii_schedule_secret');
function save_crawl_apii_schedule_secret()
{
	update_option(CRAWL_APII_OPTION_SECRET_KEY, $_POST['secret_key']);
	die();
}

add_action('wp_ajax_crawl_apii_schedule_enable', 'crawl_apii_schedule_enable');
function crawl_apii_schedule_enable()
{
	$schedule = array(
		'enable' => $_POST['enable'] === 'true' ? true : false
	);
	file_put_contents(CRAWL_APII_PATH_SCHEDULE_JSON, json_encode($schedule));
	die();
}

add_action('wp_ajax_crawl_apii_save_settings', 'crawl_apii_save_settings');
function crawl_apii_save_settings()
{
	$data = array(
	...
	if (!get_option(CRAWL_APII_OPTION_SETTINGS)) {
		add_option(CRAWL_APII_OPTION_SETTINGS, json_encode($data));
	} else {
		update_option(CRAWL_APII_OPTION_SETTINGS, json_encode($data));
	}
	die();
}

add_action('wp_ajax_crawl_apii_page', 'crawl_apii_page');
function crawl_apii_page()
{
	echo crawl_apii_page_handle($_POST['url']);
	die();
}

function crawl_apii_page_handle($url)
{
...
add_action('wp_ajax_crawl_apii_movies', 'crawl_apii_movies');
function crawl_apii_movies()
{
	$data_post 					= $_POST['url'];
	...
	$result = crawl_apii_movies_handle($url, $apii_id, $apii_update_time, $filterType, $filterCategory, $filterCountry);
	echo $result;
	die();
}

function crawl_apii_movies_handle($url, $apii_id, $apii_update_time, $filterType, $filterCategory, $filterCountry)
{
	try {
		$args = array(
			'post_type' => 'post',
		...
		if ($total > 0) { # Trường hợp đã có
			...
			if ($wp_query->have_posts()) : while ($wp_query->have_posts()) : $wp_query->the_post();
					...
					if($_halim_metabox_options["fetch_apii_update_time"] == $apii_update_time) { // Không có gì cần cập nhật
						$result = array(
							'status'   			=> true,
							'post_id' 			=> null,
							'list_episode' 	=> [],
					...
					$api_url 			= str_replace('apii.online/apii', 'apii.online/apii', $url);
					$sourcePage 	=  HALIMHelper::cURL($api_url);
					$sourcePage 	= json_decode($sourcePage, true);
					$data 				= create_data($sourcePage, $url, $apii_id, $apii_update_time);
					...
					$_halim_metabox_options["fetch_apii_update_time"] 	= $data['fetch_apii_update_time'];
		...
		$sourcePage =  HALIMHelper::cURL($api_url);
		$sourcePage = json_decode($sourcePage, true);
		$data 			= create_data($sourcePage, $url, $apii_id, $apii_update_time, $filterType, $filterCategory, $filterCountry);
...
function create_data($sourcePage, $url, $apii_id, $apii_update_time, $filterType = [], $filterCategory = [], $filterCountry = []) {
	...
	$data = array(
		'crawl_filter'						=> false,
		'fetch_url' 							=> $url,
		'fetch_apii_id' 					=> $apii_id,
		'fetch_apii_update_time' => $apii_update_time,
...
function add_posts($data)
{
	...
	$post_meta_movies = array(
		...
		'fetch_apii_id'						=> $data['fetch_apii_id'],
		'fetch_apii_update_time'		=> $data['fetch_apii_update_time'],
...
{
	// $image_url = str_replace('img.apii.online/apii', 'img.hiephanhthienha.com', $image_url);
